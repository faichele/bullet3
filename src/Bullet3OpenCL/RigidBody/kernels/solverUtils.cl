...
//	Vector
///////////////////////////////////////
__inline float fastDiv(float numerator, float denominator)
{
	return native_divide(numerator, denominator);
	//	return numerator/denominator;
}

__inline float4 fastDiv4(float4 numerator, float4 denominator)
{
	return native_divide(numerator, denominator);
}

__inline float fastSqrtf(float f2)
{
	return native_sqrt(f2);
	//	return sqrt(f2);
}

__inline float fastRSqrt(float f2)
{
	return native_rsqrt(f2);
}

__inline float fastLength4(float4 v)
{
	return fast_length(v);
}

__inline float4 fastNormalize4(float4 v)
{
	return fast_normalize(v);
}

__inline float sqrtf(float a)
{
	return native_sqrt(a);
}

__inline float4 cross3(float4 a1, float4 b1)
{
...
__inline float length3(const float4 a)
{
	return sqrtf(dot3F4(a, a));
}

__inline float dot4(const float4 a, const float4 b)
{
	return dot(a, b);
}

//	for height
__inline float dot3w1(const float4 point, const float4 eqn)
{
	return dot3F4(point, eqn) + eqn.w;
}

__inline float4 normalize3(const float4 a)
{
	float4 n = make_float4(a.x, a.y, a.z, 0.f);
	return fastNormalize4(n);
}

__inline float4 normalize4(const float4 a)
{
	float length = sqrtf(dot4(a, a));
	return 1.f / length * a;
}

__inline float4 createEquation(const float4 a, const float4 b, const float4 c)
{
	float4 eqn;
	float4 ab = b - a;
	float4 ac = c - a;
...
__inline Matrix3x3 mtZero();

__inline Matrix3x3 mtIdentity();

__inline Matrix3x3 mtTranspose(Matrix3x3 m);

__inline Matrix3x3 mtMul(Matrix3x3 a, Matrix3x3 b);

__inline float4 mtMul1(Matrix3x3 a, float4 b);

__inline float4 mtMul3(float4 a, Matrix3x3 b);
...
__inline Quaternion qtMul(Quaternion a, Quaternion b);

__inline Quaternion qtNormalize(Quaternion in);

__inline float4 qtRotate(Quaternion q, float4 vec);

__inline Quaternion qtInvert(Quaternion q);
...
__inline float4 qtInvRotate(const Quaternion q, float4 vec)
{
...
typedef struct
{
	...
	float4 m_linearAccPerContact[B3_RIGID_BODY_BEHAVIOR_MAX_CONTACTS];
	float4 m_angularAccPerContact[B3_RIGID_BODY_BEHAVIOR_MAX_CONTACTS];
...
//, __global b3RigidBodyPushPullBehavior* gPushPullBehaviors, __global b3RigidBodyBehaviorVelocities* gPushPullVelocities, int numPushPullBehaviors)
{
	...
	{
		if (invMassA) //(bodyIdA == gPushPullBehaviors[k].m_bodyID)
		{
			ppBodyId = bodyIdB;
		...
		if (invMassB) //(bodyIdB == gPushPullBehaviors[k].m_bodyID)
		{
			ppBodyId = bodyIdA;
			ppLinAcc = ppLinAcc + gPushPullBehaviors[k].m_linearAcc;
			ppAngAcc = ppAngAcc + gPushPullBehaviors[k].m_angularAcc;
							 ...
							 __global b3RigidBodyBehaviorVelocities* ppVelocities)
{
	...
	float4 dLinVelAConveyor = make_float4(0, 0, 0, 0);
	float4 dAngVelAConveyor = make_float4(0, 0, 0, 0);
	float4 dLinVelBConveyor = make_float4(0, 0, 0, 0);
	float4 dAngVelBConveyor = make_float4(0, 0, 0, 0);

	//dLinVelAConveyor.x = 3;
	//dLinVelBConveyor.x = 1;

	for (int k = 0; k < numPushPullBehaviors; ++k)
	{
		gPushPullBehaviors[k].m_linearVel.x = -10;

		/*if (gPushPullBehaviors[k].m_bodyID == aIdx)*/
		{
			// Apply to A
			dLinVelAConveyor.x = gPushPullBehaviors[k].m_linearVel.x;
			dLinVelAConveyor.y = gPushPullBehaviors[k].m_linearVel.y;
			dLinVelAConveyor.z = gPushPullBehaviors[k].m_linearVel.z;
			dAngVelAConveyor = gPushPullBehaviors[k].m_angularVel;
		}

		{
			// Apply to A
			dLinVelBConveyor.x = gPushPullBehaviors[k].m_linearVel.x;
			dLinVelBConveyor.y = gPushPullBehaviors[k].m_linearVel.y;
			dLinVelBConveyor.z = gPushPullBehaviors[k].m_linearVel.z;
			dAngVelBConveyor = gPushPullBehaviors[k].m_angularVel;
		}

		//if (gPushPullBehaviors[k].m_bodyID == bIdx)
		//{
		//	// Apply to B
		//	dLinVelBConveyor = gPushPullBehaviors[k].m_linearVel;
		//	dAngVelBConveyor = gPushPullBehaviors[k].m_angularVel;
		//}
	}

	//dLinVelAConveyor.x = 0;
	//dLinVelBConveyor.x = 0;

	//dLinVelAConveyor.y = 0;
	//dLinVelBConveyor.y = 0;

	//dLinVelAConveyor.z = 0;
	//dLinVelBConveyor.z = 0;
...

	{
...

		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				float rambdaDt = calcRelVel(linear, -linear, angular0, angular1, linVelA + dLinVelA + dLinVelAConveyor, angVelA + dAngVelA + dAngVelAConveyor, linVelB + dLinVelB + dLinVelBConveyor, angVelB + dAngVelB + dAngVelBConveyor
...
__kernel void UpdateBodyVelocitiesKernel(__global Body* gBodies, __global int* offsetSplitBodies, __global const unsigned int* bodyCount,
										 __global float4* deltaLinearVelocities, __global float4* deltaAngularVelocities, __global b3RigidBodyBehaviorVelocities* gPushPullVelocities,
										 int numBodies)
{
	int i = GET_GLOBAL_IDX;
	if (i < numBodies)
...
__kernel
	__attribute__((reqd_work_group_size(WG_SIZE, 1, 1))) void
	ContactToConstraintSplitKernel(__global const struct b3Contact4Data* gContact, __global const Body* gBodies, __global const Shape* gShapes, __global Constraint4* gConstraintOut,
